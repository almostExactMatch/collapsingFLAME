...
def run_bit(df, holdout, covs, covs_max_list,covs_unimportant,threshold, tradeoff_param = 0.1):
...
#            break
        ...
        balancing_F, time_BF = balancing_factor(df, best_res[3], tradeoff=tradeoff_param)
        balance.append(balancing_F)
        prediction_E, time_PE, n_mse_T, n_mse_C = prediction_error(holdout, cur_covs)
        
        n_mse_treatment.append(n_mse_T)
        n_mse_control.append(n_mse_C)

        prediction.append(prediction_E)
        ...
        if len(cur_covs) == len(covs) - covs_unimportant: 
            print('all important covariate dropped')  
            break
   
            
    return (timings, cleanup_result(matching_res), nb_steps, level_scores, nb_match_units, drops, balance, prediction, n_mse_treatment, n_mse_control, nb_units_c, nb_units_t )
